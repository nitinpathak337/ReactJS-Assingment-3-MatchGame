...
class App extends Component {
  ...
  updateScore = imageUrl => {
    ...
    if (imageUrl === imagesList[randomNumber].imageUrl) {
  ...
  display = () => {
    const {randomNumber, category} = this.state
    const src = imagesList[randomNumber].imageUrl
    ...
    return (
      <>
        <img src={src} alt="match" className="match-image" />
  ...
  render() {
    const {score, sec, gameOver} = this.state
    ...
    return (
      <div className="bg">
        ...
        {gameOver ? this.gameOverCard() : this.display()}
      </div>
    )
  }
}
